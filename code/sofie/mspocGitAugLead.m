function [Wx, Wy, Wtau, Ax, Ay, out] = mspocGitAugLead(X, Y,G,gamma,varargin)
...
opt = set_defaults(opt, ...
    ...
    'verbose',1, ...
    'component_idx', 1, ...
    'Ay_cut',1,... % threshold the fMRI pattern
    'is_white_y', 0 ...
    );
...
%         [wt, wy] = my_reg_CCA(Pxe(:,mask), Y(:,mask), kappa_tau, kappa_y, Dpp, Dyy, Cyy);
        ...
        for signs=1:2
            ...
            [W, D] = eig(c*(1-gamma)*Cxxz+(gamma)*Caa, Cxx);
            maxD(signs)=max(diag(D)); %#ok<AGROW>
        end
        if max(maxD)==maxD(1)
            [W, D] = eig((1-gamma)*Cxxz+(gamma)*Caa, Cxx);
        end
