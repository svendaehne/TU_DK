function [corr_TE, corr_TR] = calcCorrLead(Cxxe,Y,wx,wy,wt,idx,tr_idx)
% Inputs:
...
corrtmp = corrcoef(sy_est(val_idx), px_flt_est(val_idx));
corr_TE = (corrtmp(1,2));

% corr temp, est filt sx and sy_est, train
corrtmp = corrcoef(sy_est(tr_idx), px_flt_est(tr_idx));
corr_TR = (corrtmp(1,2));
